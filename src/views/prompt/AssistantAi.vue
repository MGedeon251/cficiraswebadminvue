        ...
        <div class="flex-grow-1 overflow-auto p-2">
          <div class="list-group list-group-flush">
            <button
              v-for="h in history"
              :key="h.id"
              class="list-group-item list-group-item-action border-0 rounded mb-1 small"
            >
      ...
      <div class="col-md-9 d-flex flex-column bg-white">
        <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
          <div class="d-flex align-items-center">
            <div
              class="avatar-ia bg-dark text-white rounded-circle d-flex align-items-center justify-content-center me-3"
            ...
            <div>
              ...
              <small class="text-success"
                ><i class="mdi mdi-circle ms-0" style="font-size: 8px"></i> Connecté aux données en
                temps réel</small
              >
            </div>
          </div>
        ...
        <div class="chat-body flex-grow-1 p-4 overflow-auto" ref="chatBox">
          <div class="d-flex mb-4">
            <div class="message-bubble assistant p-3 rounded shadow-sm border bg-light">
              <p class="mb-2">
                Bonjour Administrateur. Je suis prêt à analyser l'ensemble des opérations de la
                plateforme.
              </p>
              <p class="mb-0 small text-muted">Exemples de questions que vous pouvez me poser :</p>
              <div class="mt-2 d-flex flex-wrap gap-2">
                <button
                  @click="quickQuestion('Quel est le bénéfice net de ce mois ?')"
                  class="btn btn-xs btn-outline-dark rounded-pill py-1 px-2 small-text"
                >
                  Bénéfice net du mois ?
                </button>
                <button
                  @click="quickQuestion('Liste des impayés en Licence 1')"
                  class="btn btn-xs btn-outline-dark rounded-pill py-1 px-2 small-text"
                >
                  Impayés L1
                </button>
                <button
                  @click="quickQuestion('Prévision des honoraires pour le mois prochain')"
                  class="btn btn-xs btn-outline-dark rounded-pill py-1 px-2 small-text"
                >
                  Prévisions Honoraires
                </button>
              </div>
            </div>
          </div>
          ...
          <div
            ...
            :key="index"
            :class="['d-flex mb-4', msg.role === 'user' ? 'justify-content-end' : '']"
          >
            <div
              :class="[
                'message-bubble p-3 rounded shadow-sm',
                msg.role === 'user' ? 'bg-dark text-white user' : 'bg-light border assistant',
              ]"
              style="max-width: 80%"
            >
          ...
          <div v-if="isTyping" class="text-muted small mb-3 italic">
            <span class="spinner-grow spinner-grow-sm me-1"></span> L'IA analyse les bases de
            données...
          </div>
        </div>
        ...
        <div class="p-4 border-top">
          ...
          <p class="text-center text-muted mt-2 mb-0" style="font-size: 10px">
            L'IA peut faire des erreurs. Vérifiez les chiffres importants dans les rapports
            officiels.
          </p>
        </div>
      </div>
