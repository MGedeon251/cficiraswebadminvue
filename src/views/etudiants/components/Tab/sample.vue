...
export default {
  ...
  setup() {
    ...
    const exportExcel = () => {
      ...
      console.log('Export Excel des étudiants:', filteredEtudiants.value);
    };
    ...
    const exportPDF = () => {
      ...
      // === BANNIÈRE SUPÉRIEURE ===
      // Rectangle gris clair en haut
      doc.setFillColor(240, 240, 240);
      doc.rect(0, 0, pageWidth, 45, 'F');

      // Texte à gauche (informations du centre)
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);

      doc.text("CENTRE D'INFORMATIQUE ET DE", 14, 12);
      doc.text("RECHERCHE DE L'ARMÉE ET DE", 14, 18);
      doc.text('LA SÉCURITÉ', 14, 24);

      doc.setFontSize(9);
      doc.text('*********', 14, 28);

      doc.setFontSize(10);
      doc.text('CENTRE DE FORMATION EN', 14, 34);
      doc.text('INFORMATIQUE', 14, 40);

      // Ligne verticale de séparation
      doc.setDrawColor(100, 100, 100);
      doc.setLineWidth(0.5);
      doc.line(pageWidth / 2 - 20, 8, pageWidth / 2 - 20, 42);

      // Texte à droite (SERVICE)
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10);
      const serviceText = 'SERVICE DES AFFAIRES';
      const serviceText2 = 'PEDAGOGIQUES';
      const serviceX = pageWidth / 2 - 15;

      doc.text(serviceText, serviceX, 20);
      doc.text(serviceText2, serviceX, 26);
      doc.setFontSize(9);
      doc.text('********', serviceX, 32);

      // Logo (placeholder - à remplacer par votre logo)
      // Si vous avez un logo en base64, décommentez et utilisez:
      // doc.addImage(logoBase64, 'PNG', pageWidth - 50, 8, 35, 30);

      // Texte "Excellence - Qualité - réussite" sous le logo
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(8);
      doc.setTextColor(200, 50, 50);
      doc.text('Excellence - Qualité - réussite', pageWidth - 52, 42);

      // === TITRE DU DOCUMENT ===
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      doc.text('LISTE DES ÉTUDIANTS PAR CLASSE', pageWidth / 2, 55, { align: 'center' });
      ...
      doc.text(`Année académique: ${anneeText}`, 14, 63);
      doc.text(`Filière: ${filiereText}`, 14, 68);
      doc.text(`Classe: ${classeText}`, 14, 73);
      doc.text(`Total étudiants: ${filteredEtudiants.value.length}`, pageWidth - 14, 63, {
        align: 'right',
      });
      doc.text(`Date d'édition: ${new Date().toLocaleDateString('fr-FR')}`, pageWidth - 14, 68, {
        align: 'right',
      });
