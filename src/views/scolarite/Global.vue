<template>
    <div>
      ...
      <section class="stats-top">
        <div class="stat-block">
          ...
          <h3>Classes</h3>
          <p><strong>{{ classes.length }}</strong> classes créées</p>
          <ul>
        ...
        <div class="stat-block">
          ...
          <h3>Étudiants</h3>
          <p>Nombre total : <strong>{{ totalEtudiants }}</strong></p>
          <p>Hommes : <strong>{{ nbHommes }}</strong></p>
          <p>Femmes : <strong>{{ nbFemmes }}</strong></p>
        </div>
        ...
        <div class="stat-block">
          ...
          <p>Hommes : <strong>{{ pourcentageHommes.toFixed(1) }}%</strong></p>
          <p>Femmes : <strong>{{ pourcentageFemmes.toFixed(1) }}%</strong></p>
        </div>
      </section>
  ...
  onMounted(async () => {
    try {
      const response = await axios.get('/api/scolarite/global')
  
      // Exemple attendu du backend
      // {
      //   classes: [
      //     { id: 1, nom: 'Classe A', nbPlaces: 30, nbEtudiants: 20, nbGarcons: 12, nbFilles: 8 },
      //     { id: 2, nom: 'Classe B', nbPlaces: 25, nbEtudiants: 15, nbGarcons: 6, nbFilles: 9 }
      //   ],
      //   etudiants: {
      //     total: 35,
      //     hommes: 18,
